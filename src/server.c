...
int init_server_settings(server_settings *setts)
{
	...
	while (!feof(configFile)) {
		switch ((char) fgetc(configFile)) {
			...
			case '\n': // emptyline
				break;
			default:CHECK_PERROR_EXIT(fseek(configFile, -1, SEEK_CUR) == -1,
...
int serve(int request, int client_socket, u_file_storage *storage)
{
	...
	switch (op)
	{
		case SAPI_OPENFILE: return server_openFile(request, client_socket, storage);
		case SAPI_CLOSEFILE: return server_closeFile(request, client_socket, storage);
		case SAPI_READFILE: return server_readFile(request, client_socket, storage);
		case SAPI_READNFILES: return server_readNFiles(request, client_socket, storage);
		case SAPI_WRITEFILE: return server_writeFile(request, client_socket, storage);
		case SAPI_APPENDFILE: return server_appendToFile(request, client_socket, storage);
		case SAPI_LOCKFILE: return server_lockFile(request, client_socket, storage);
		case SAPI_UNLOCKFILE: return server_unlockFile(request, client_socket, storage);
		case SAPI_REMOVEFILE: return server_removeFile(request, client_socket, storage);
		default:fprintf(stderr, "serve : unrecognized operation\n");
			fflush(stderr);
			return -1;
	}
	return 0;
